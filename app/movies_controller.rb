...
def can_get_size_of_the_database
  Movie.all.length #or Movie.count
end
...
def can_find_using_where_clause_and_be_sorted
  ...
  #Movie.find(:release_date < 2002) this is what i originally did
  # Why doesn't release date have colons?
  Movie.where("release_date > 2002").order(release_date: :desc)
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick", save it, then return it
  #Why is movie variable changed?
...
def can_update_multiple_items_at_once
  ...
  #My begining attempt
  #Movie.update(title: "Movie_#{i}")
  # Why doesn't this work? Movie.all.each {|movie| movie.title = "A movie"}
